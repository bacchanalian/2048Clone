            ...
            if i > 0:  # make it so nothing happens when cursor is at top most row
                for j in range(0, L, 1):  # start counting along columns to the right
                    if temparray[i][j] == 0:  # go to next column if this is 0
                        continue
                    if [i, j] in templist or [i - 1,
                                              j] in templist:  # go to next column if block, position or next position above has previously been doubled
                        continue
            ...
            if j > 0:  # make it so nothing happens when cursor is at the left most column
                for i in range(0, H, 1):  # start counting down rows
                    if temparray[i][j] == 0:  # go to next row if this is 0
                        continue
                    if [i, j] in templist or [i,
                                              j - 1] in templist:  # go to next row if block, position or next position to right has previously been doubled
                        continue
    ...
    def canmove(matrix):
        ...
        for i in range(0, H, 1):
            for j in range(0, L, 1):
                if (matrix[i][j] == 0) & ([i, j] not in block):
                    return True
        if (matrix == right(matrix, block)) & (matrix == left(matrix, block)) & (matrix == up(matrix, block)) & (
            matrix == down(matrix, block)):
            return False
        return True
